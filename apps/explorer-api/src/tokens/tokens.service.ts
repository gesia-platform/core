...
import {
  ListTokensRequestQueryDto,
  ListTokensResponseDto,
} from './dtos/list-tokens.dto';
...
export class TokensService {
  ...
  async getFTData(chainID: number, address: string) {
    ...
    const symbol = provider.eth.abi.decodeParameter(
      'string',
      symbolABI,
    ) as string;
    ...
    const totalSupply = BigInt(
      provider.eth.abi.decodeParameter('uint256', totalSupplyABI) as any,
    );
    ...
    const holders = transferLogs.reduce((p: any, x: any) => {
      const from = provider.eth.abi.decodeParameter(
        'address',
        x.topics[2],
      ) as string;

      const to = provider.eth.abi.decodeParameter(
        'address',
        x.topics[2],
      ) as string;
  ...
  async getNFTData(chainID: number, address: string) {
    ...
    const voucherAddress = provider.eth.abi.decodeParameter(
      'address',
      voucherAddressABI,
    ) as string;
  ...
  async listTokens(
    request: ListTokensRequestQueryDto,
  ): Promise<ListTokensResponseDto> {
    const func =
      request.type === 'ft'
        ? this.web3Service.getFTAddresses
        : this.web3Service.getNFTAddresses;

    const addresses = func(
      Number(request.chainID),
      Number(request.pageOffset),
    ...
    const results = await Promise.all(
      addresses.map((address) =>
        request.type === 'ft'
          ? this.getFTData(Number(request.chainID), address)
          : this.getNFTData(Number(request.chainID), address),
      ),
    );

...
import {
  ListVouchersRequestQueryDto,
  ListVouchersResponseDto,
} from './dtos/list-vouchers.dto';
import { GetVoucherRequestQueryDto } from './dtos/get-voucher.dto';
import { Web3Service } from 'src/web3/web3.service';
import { VoucherDto } from './dtos/voucher.dto';
import Web3 from 'web3';
import {
  ListVoucherTokensRequestQueryDto,
  ListVoucherTokensResponseDto,
} from './dtos/list-voucher-tokens.dto';
...
export class VouchersService {
  ...
  async getVoucherData(chainID: number, address: string) {
    ...
    const tokenIds = new Set(
      mintLogs.map((x: any) => {
        ...
        return BigInt(
          provider.eth.abi.decodeParameter('uint256', datas[0]) as any,
        ).toString();
      }),
    );
  ...
  async listVoucherTokens(
    voucherID: string,
    request: ListVoucherTokensRequestQueryDto,
  ): Promise<ListVoucherTokensResponseDto> {
  ...
  async listVouchers(
    request: ListVouchersRequestQueryDto,
  ): Promise<ListVouchersResponseDto> {
    const addresses = this.web3Service.getVoucherAddresses(
    ...
    const results = await Promise.all(
      addresses.map((address) =>
        this.getVoucherData(Number(request.chainID), address),
      ),
    );

...
import { ListBlocksRequestQueryDto, ListBlocksResponseDto } from './dtos/list-blocks.dto';
...
export class BlocksService {
    ...
    constructor(@InjectModel(Block.name) private blockModel: Model<Block>, private web3Service: Web3Service) {}
    ...
    async getLatestBlock(networkID: number) {
        const block = await this.blockModel.findOne({ networkID }, {}, { sort: { height: -1 } });
        return block;
    }

    async getBlock(blockID: string, query: GetBlockRequestQueryDto, recursive?: boolean) {
        const pipelines: PipelineStage[] = [
            { $match: { chainID: Number(query.chainID) } },
            {
                $match: isObjectIdOrHexString(blockID) ? { _id: new mongoose.Types.ObjectId(blockID) } : { height: blockID },
            },
            {
                $lookup: {
                    from: 'txs',
        ...
        if (!results[0]) {
            if (!recursive) {
                await this.web3Service.processBlock(this.web3Service.getProvider(Number(query.chainID)), Number(query.chainID), blockID);
    ...
    async listBlocks(query: ListBlocksRequestQueryDto): Promise<ListBlocksResponseDto> {
        ...
        // for (let i = latestBlockNumber - skip; i > latestBlockNumber - skip - limit; i--) {
        //     heights.push(i);
        // }

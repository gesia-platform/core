...
contract CarbonAirPlaneCalculator is CarbonEmissionsCalculatorBase {
    ...
    function calculateCO2ByAirPlane(bool isBusiness, uint256 distance, uint256 p, uint256 seat, uint256 k, uint256 averageFuelBurn, uint256 applicationId, address carbonEmissions, string memory sourceChannel) external returns (uint256) {
        uint256 classType = 1;
        if (isBusiness) {
            classType = 2;
        }

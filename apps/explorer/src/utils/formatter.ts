...
export const formatAddress = (address: string) => address?.substring(0, 13) + '...' + address?.substring(address.length - 10);

export const formatHash = (hash: string) => hash.substring(0, 15) + '...';

export const formatTCO2 = (value: string, isEmission: boolean) => {
	return ((isEmission ? Number(BigInt(value) / BigInt(1000000000)) : Number(BigInt(value))) / 1000).toFixed(2);
};
...
export const formatNZC = (data: bigint | undefined | null) => {

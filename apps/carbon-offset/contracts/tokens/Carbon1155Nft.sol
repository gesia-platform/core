...
contract Carbon1155Nft is ERC1155, ReentrancyGuard, IPrice {
    ...
    function mint(
        ...
        // company fee amount
        uint256 calculatedAmount = IFeeManager(feeManager).carbonMintFeeAmount(_carbonAmount);
    ...
    function transferWithSignature(
        ...
        transactionHashes[hashMessage] = true;
        // company fee amount
        uint256 calculatedAmount = IFeeManager(feeManager).carbonTransferFeeAmount(amount);
    ...
    function transferWithGift(
        ...
        transactionHashes[hashMessage] = true;
        // company fee amount
        uint256 calculatedAmount = IFeeManager(feeManager).carbonGiftFeeAmount(amount);
        uint256 remainAmount = amount.sub(calculatedAmount);
        // transfer fee to company owner
        _safeTransferFrom(from, IFeeManager(feeManager).feeAddress(), tokenId, calculatedAmount, "");
        _safeTransferFrom(from, to, tokenId, remainAmount, "");
    ...
    function burnByOwner(address from, uint256 tokenId, uint256 amount) external operatorsOnly {
